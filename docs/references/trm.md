...
**"少即是多"的递归推理**：**单网络 + 2 层 + 5M 参数**在 **Sudoku-Extreme** 上达到 **87.4%**（vs HRM 27M 55.0%），在 **ARC-AGI-1** 上达到 **44.6%**（TRM-Att 7M vs HRM 27M 40.3%）——只用 0.01% 的 LLM 参数。**直接挑战 HRM 的"双网络分层"必要性**。
...
### 4.1 Sudoku-Extreme（论文 Table 1 — 消融）

| 模型 | Sudoku-Extreme 准确率 | 深度 | NFP | 参数 |
|---|---|---|---|---|
| **HRM** | 55.0% | 24 | 2 | 27M |
...
- Self-attention 也**降低**性能（74.7%）→ **MLP 更适合 Sudoku**

### 4.2 ARC-AGI 基准（论文 Table 5 — 主结果，2 tries）

| 模型 | 参数 | ARC-AGI-1 | ARC-AGI-2 |
|---|---|---|---|
| **HRM** | 27M | 40.3% | 5.0% |
| **TRM-Att (Ours)** | **7M** | **44.6%** | **7.8%** |
| TRM-MLP (Ours) | 5M | — | — |
| DeepSeek R1 671B | 671B | 15.8% | 1.3% |
| Claude 3.7 16K | — | 28.6% | 0.7% |
| o3-mini-high | — | 34.5% | 3.0% |
| Gemini 2.5 Pro 32K | — | 37.0% | 4.9% |

**关键观察**（主结果）：
- TRM-Att 7M 在 ARC-AGI-1 上 **超越 HRM 27M 4.3 个百分点**（44.6 vs 40.3）
- TRM-Att 7M 超过所有列出的 LLM（包括 671B DeepSeek R1）
- 论文摘要（"45% test-accuracy on ARC-AGI-1"）和这里表格数字（44.6%）在四舍五入层面一致
