...
以下是 GRR-300M 与当前主流 RRM 范式（HRM-Text, GRAM, TRM/PTRM, Looped Transformer）的深度技术对比分析。

### 1. 核心技术维度对比矩阵

| 维度 | GRR-300M (本方案) | HRM-Text (清华/智源) | GRAM (Bengio/Mila) | TRM / PTRM | Looped Transformer |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
| **循环控制** | **三重自适应**：Exit Probe + Token Router + Adaptive Reset | **固定日程**：预设 H2L3 等固定循环比 | **并行扩展**：推理时采样 K 条轨迹取优 | **外部探针**：PTRM 引入独立退出分类器 | **固定深度**：训练/推理均为固定轮数 |
...
#### 🆚 vs. TRM/PTRM：系统化 vs. 原型化

-   **TRM 的贡献**：证明了 7M 参数即可在数独上达到 87%，确立了“递归精炼”的可行性下限。
-   **PTRM 的局限**：其退出探针是**事后外挂**的，与精炼过程解耦。这意味着探针无法影响精炼方向，只能被动判断“够不够好”。
...
| **PTRM**: 坏盆地无法逃逸 | Confidence-driven Adaptive Reset | Reset 阈值 τ 的 Curriculum 设计缺乏理论指导 |
...
-   相比 TRM/PTRM，它增加了架构复杂度，换来了**生产环境所需的鲁棒性与可控性**。
