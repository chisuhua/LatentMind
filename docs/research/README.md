...
> **组织形式**：三研究线（Logos / SADKO / ELF）+ 顶级索引
...
| **SADKO 主干** | [`sadko/`](./sadko/) | 感知 + 记忆 + 知识 + 多模态 | 异构双脑（AR + ELF）+ 扩散桥梁 | ✅ 2026-07-29 升级 |
| **ELF 独立研究线** | [`elf/`](./elf/) | 右脑关键技术独立验证（记忆 / KG / 检索）| 复用 ELF 机制（双向 + FM + FSQ）| 🆕 2026-07-30 启动 |

**双轨 + 探索分支**：
- Logos + SADKO = 项目双轨分工（详见 [AGENTS.md §7](../../AGENTS.md#7-研究路线分工双轨制--2026-07-29-战略决策)）
- ELF = SADKO 右脑关键技术的独立延伸（详见 [elf/README.md](./elf/README.md)）
...
- **ELF 独立研究线**：见 [`elf/README.md`](./elf/README.md)（含胼胝体接口契约五元组）
...
2. **跨子目录引用**：使用相对路径（`./`、`../sadko/`、`../logos/`、`../elf/`）
3. **跨研究线协调文档**：放 logos/ 下作为 Logos 主导的协调（如 logos/sadko-64m-coordination.md）
4. **接口契约变更**：ELF 研究线的胼胝体契约变更需双侧 review（详见 [elf/README.md §2.3](./elf/README.md#23-关键不变量)）
5. **语言**：中文为主，英文技术术语保留
