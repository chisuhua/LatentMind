...
> **一句话定位**：在 v1.0 最优基座（A.3 混合风格应最优）上对比五种循环策略——串行、PLT、Per-Token 早退、Radix Cache 多路径、层次化推理，绘制"延迟-精度" Pareto 曲线，找到 Logos 端侧的最优循环策略组合。
> **上游文档**：[logos-64m-validation-plan.md §3](./logos-64m-validation-plan.md#3-v20多种循环策略对比)
> **下游文档**：[logos-v3-architecture.md](./logos-v3-architecture.md)（基于 v2.0 推荐循环策略）
> **整合**：[RDD-0001-k-sweep-experiment.md](../rfcs/RDD-0001-k-sweep-experiment.md) 已合并到 §B.1
> **最后更新**：2026-07-29
...
| 策略 | 延迟 | 突破 K=2 的机制 | 适用场景 |
|------|------|---------------|---------|
| **B.1 串行 K** | K × 1x | 无（基线）| K ≤ 2 |
...
- B.3 早退平均 K ≤ 3
- B.4 Radix Cache 4 路径延迟 < 1.5x K=2
- B.5 层次化延迟 < 3x K=1
- **核心结论**：三件套组合覆盖所有场景
...
**RDD-0001 整合**：详细 K-sweep 实验设计见 [RDD-0001-k-sweep-experiment.md](../rfcs/RDD-0001-k-sweep-experiment.md)（已合并到本节）。
...
def hlt_plt(x, H_cycles=2, L_cycles=3):
    ...
    for h in range(H_cycles):
        ...
        for l in range(L_cycles):
            h_L_shifted = shift_right(h_L)  # CLP 偏移
            h_L_input = h_L_shifted + h_H
            h_L = parallel_L_block(h_L_input)  # 一次 forward 并行所有 L
        # H block 串行（保持慢变量）
        h_H = H_block(h_H + h_L)
...
- H 循环必须串行（H 是慢速全局）
- L 循环可并行（L 是快速局部）
- 需要 L_block 支持并行 chain（修改 L_module 添加 CLP 偏移）
...
**风险**：CLP 位置偏移税（[loopcoder-v2.md §3.2](../references/loopcoder-v2.md#32-clpcross-loop-position-shift)）可能导致 L 循环精度损失。
...
**延迟节省**：平均循环数 ≤ 3（基于 Per-Token Convergence 数据）。
...
**风险**：早退阈值 ε 需调优，过激进损失精度，过保守无收益。

### 3.4 B.4 Radix Cache 多路径并行

**借鉴自**：你提出的方案（详见 [logos-k-strategy.md §4.3](./logos-k-strategy.md#43-radix-cache-多路径并行你提出的方案)）
...
def radix_cache_multipath(x, n_paths=4, K=2):
    ...
    for i in range(n_paths):
        ...
        h_i = base_h.clone()
        for k in range(K):
            # 检索 Radix Cache（前缀 hash 命中则复用）
            cached = radix_cache.get(h_i.hash())
            if cached is not None:
                h_i = cached
                continue
            h_i = hrm_block(h_i, x, eps_i)
...
**关键优化**：
- 路径并行：N 条路径在同一 forward 内并行
- Radix Cache：共享前缀不重复计算
- 综合策略：平均 / 投票 / 加权（端侧可选）
...
**借鉴自**：你提出的方案（详见 [logos-k-strategy.md §4.4](./logos-k-strategy.md#44-层次化推理你提出的另一个方案)）
...
def hierarchical_reasoning(x, K_main=2, K_sub=1):
    ...
    # 主推理第一阶段
    h_main_1 = hrm_main_block(h, x)  # K_main 第一阶段
    
    # 触发子推理（并行）
    sub_results = parallel([
        hrm_sub_block_1(h_main_1, x),
        hrm_sub_block_2(h_main_1, x),
        hrm_sub_block_3(h_main_1, x),
    ])  # K_sub 步并行
    
    # 主推理第二阶段（融合子结果）
    h_main_2 = hrm_main_block(h_main_1, x, sub_context=sub_results)
...
**适用场景**：可分解推理（数学先计算再验证、代码先生成再检查）。

**挑战**：需要设计"何时触发子推理"的机制（可学习门控 vs 固定位置）。
...
| B.4 Radix Cache 4 路径 K=2 | 2 | 4 | 8 | **~100ms** | 多假设决策 +M% |
...
延迟 < 100ms 预算：
  ✅ B.1 K=1（50ms）
  ✅ B.1 K=2（100ms）
  ✅ B.4 Radix Cache 4 路径 K=2（~100ms）
...
- **B.1 K=2**：简单任务基线（实时对话）
- **B.3 早退**：动态 K（通用加速）
- **B.4 Radix Cache**：多假设决策（GR AM 端侧化）

**扩展**：
- **B.2 PLT**：v1.5 升级时用于 K=4-8 推理
...
| v2.0 结论 | v3.0 基线 |
|----------|---------|
| 三件套最优（B.1 K=2 + B.3 + B.4）| 用三件套作为 v3.0 基线 |
...
| B.4 Radix Cache 实现复杂 | v3.0 用 GRAM 串行多轨迹 |
| B.5 层次化难以端到端 | 不集成层次化 |
...
| 文档 | 关系 |
|------|------|
| [logos-64m-validation-plan.md §3](./logos-64m-validation-plan.md#3-v20多种循环策略对比) | 本文档的父级 |
| [logos-v1-architecture.md](./logos-v1-architecture.md) | v1.0 双时间尺度对比（v2.0 基座）|
| [logos-k-strategy.md §4](./logos-k-strategy.md#4-多种端侧并行化策略latency-killer) | 五策略核心论证 |
| [logos-whitepaper.md §2.3](./logos-whitepaper.md#23-模块-bgram-多轨迹扰动) | GRAM + Radix Cache 高层架构 |
| [docs/rfcs/RDD-0001-k-sweep-experiment.md](../rfcs/RDD-0001-k-sweep-experiment.md) | K-sweep RFC（已合并到 §B.1）|
