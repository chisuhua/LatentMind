...
| **backbone** | 分层双时间尺度循环（H/L 借鉴，独立设计） | AR-Native + Split-GQA + Hippo（独立设计） |
...
**融合接口**：SADKO 训练后冻结的 Hippo Memory KV 通过 **Cross-Attention** 喂给 Logos 主线 H 模块的中间层（"HRM 是大脑皮层，Hippo 是海马体"）。
...
**与 SADKO 的差异**：Logos 的感知层是**单向前馈**（简单 3 层 CNN），不做循环；SADKO 的 Hippo 是**双向 + FM** 流形压缩——这是"快速感知 vs 流形记忆"的分工。
...
│  接收：感知主干输出 + SADKO Hippo Memory KV (via Cross-Attention) │
│  输出：动作 / 决策 / 结构化解释                                  │
└─────────────────────────┬────────────────────────────────────┘
                          │ Cross-Attention 融合
                          ↓
...
**关键比喻**："**HRM 是大脑皮层，Hippo 是海马体**"——Logos 负责"思考"，SADKO 负责"记忆与感知"。
...
| **感知层多模态融合失败** | 🟠 中 | 中 | 切换到 SADKO Hippo 双向感知主干（融合接口）|
