...
> **最后更新**：2026-07-31（v1.5：新增 §3.6 Loop B 备选实验 + §4.6 L-PLT 评估 + §4.6.9 L-PLT+Φ 联合实验）
...
#### 4.6.9 L-PLT + Φ 联合实验（延迟 + 质量联合优化）🆕 v1.4

> 📌 **2026-07-31 新增**：基于 [k-strategy §2.2.7](../k-strategy.md) 发现的 **PLT 跨位置对齐**与 **DiscoLoop 循环内对齐**的本质共性，本节设计联合实验验证 L-PLT 与 Φ 通道的协同效果。

##### 4.6.9.1 核心命题

**L-PLT（延迟优化，33% 节省）与 Φ 通道（质量优化，OOD +50%）能否叠加？叠加后是否保持各自优势？**

**关键背景**（[k-strategy §2.2.7.1](../k-strategy.md)）：
- DiscoLoop 对齐：**纵向漂移**（同位置多轮后 H 偏离 E 分布）
- PLT 跨位置对齐：**横向错位**（不同位置同一轮间 shift(h) 偏离 E 分布）
- 两者本质相同（f_θ 消费分布失配），但触发维度正交
- Φ 通道可应用于**任意维度**的对齐——是"通用分布修复算子"

##### 4.6.9.2 三种 Φ 集成位置（基于 §4.6.6 P.0/P.1）

| 实验 | Φ 位置 | 计算开销 | 端侧延迟影响 |
|------|--------|:---:|:---:|
| **P.0 HLT-PLT** | ❌ 无 Φ | 0× | 基线 |
| **P.1 L-PLT** | ❌ 无 Φ | 0× | -33%（仅 L-PLT 本身）|
| **Q.1 L-PLT + Φ 输入对齐** | shift 前 | 1× O(T × V × d) | -33% + 中（额外 Φ 一次）|
| **Q.2 L-PLT + Φ 输出对齐** | L_module 输出后 | 1× O(T × V × d) | -33% + 中 |
| **Q.3 L-PLT + Φ 双端对齐** | 输入 + 输出 | 2× O(T × V × d) | -33% + 高（额外 Φ 两次）|
...
def l_plt_with_phi(h_L_prev, h_H, l):
    ...
    L-PLT + Φ 输入对齐（最实用方案）
    """
    h_L_shifted = shift_right(h_L_prev)           # 跨位置 shift
    h_L_aligned = phi(h_L_shifted)                # ⭐ Φ 输入对齐
...
```
§3.5 探针结果 + §4.6.8 P.1 评估结果
   ↓
判断：是否需要 L-PLT + Φ 联合验证？
   ├─ §3.5 错位不显著 + §4.6.8 P.1 失败 → ❌ 不启动 Q.Φ
   ├─ §3.5 错位不显著 + §4.6.8 P.1 启用 → ⚠️ 仅启用 P.1（无 Φ）
   ├─ §3.5 错位显著 + §3.5 Φ 通道有效 + §4.6.8 P.1 启用 → ✅ 启动 Q.Φ
   └─ §3.5 错位显著 + §3.5 Φ 通道无效 → 需 §3.6 Loop B fallback
```
...
| 指标 | 计算方式 | 通过标准 | 触发动作 |
|------|---------|:---:|------|
| **端侧延迟** | 单 token ms | K=4 ≤ 150ms（与 P.1 持平）| > 200ms → 标记"Φ 开销过大" |
| **加速比 vs P.0** | vs baseline 串行 | ≥ 33%（与 P.1 持平）| < 25% → 标记"Φ 抵消 L-PLT 收益" |
| **加速比 vs P.1** | vs P.1 (L-PLT 无 Φ) | ≥ 95%（Φ 开销 ≤ 5%）| < 90% → 标记"Φ 开销过大" |
| **OOD 多跳准确率** | entity-disjoint 2-hop | ≥ 0.85（与 §3.5 单独 Φ 相当）| < 0.7 → 标记"Φ 失效" |
| **PPL 退化** | vs P.1 (L-PLT 无 Φ) | < 2%（Φ 修复错位）| > 5% → 标记"质量未改善" |
| **桥接实体 cosine** | per-boundary cosine | ID ≥ 0.5（与 §3.5 baseline 相当）| < 0.4 → 标记"对齐失效" |

##### 4.6.9.5 与 §3.5 §4.6 共享 Φ 模块

**关键架构洞察**：Q.Φ 不是引入新模块，而是**复用 §3.5 验证过的 Φ 模块**：
...
└────────────────────────────────────────────────┘
            ↓
┌─────────────────────┐  ┌─────────────────────┐
│ §3.5 DiscoLoop 探针 │  │ §4.6 L-PLT           │
│ 纵向对齐（同位置）  │  │ 横向对齐（跨位置）  │
│ 应用：H^(k) + E     │  │ 应用：shift(h) + E   │
└─────────────────────┘  └─────────────────────┘
```
...
##### 4.6.9.6 实施决策（开始前必须锁定）

| 决策 | 选项 | 推荐 |
|------|------|------|
| Φ 集成位置 | (a) 输入对齐 (b) 输出对齐 (c) 双端 | **(a) 输入对齐**——开销小且对齐完整 |
| Φ 共享 vs 独立 | (a) 与 §3.5 共享 (b) 独立模块 | **(a) 共享**——降低架构复杂度 |
| α gate 设置 | (a) 固定 1.0 (b) 可学习 | **(a) 固定 1.0**——与 DiscoLoop 原文一致 |
| 与 B.3 早退关系 | (a) 启用 Q.Φ (b) Q.Φ + 早退 | **(a) 启用 Q.Φ**——避免双重优化干扰 |

##### 4.6.9.7 实验时序与门禁

```
§3.5 Φ 验证通过 + §4.6.8 P.1 启用
   ↓
启动 Q.1 L-PLT + Φ 输入对齐
   ├─ 端侧延迟 vs P.1 ≥ 95% → ✅ Φ 开销可接受
   │   ├─ OOD 准确率 > 0.85 → ✅ 启用 Q.Φ（延迟 + 质量联合优化）
   │   └─ OOD 准确率 ≤ 0.85 → ❌ Φ 失效，保留 P.1
   ├─ 端侧延迟 vs P.1 < 95% → ⚠️ 评估 Q.2（输出对齐是否开销小）
   │   ├─ Q.2 ≥ 95% 且 OOD > 0.85 → ✅ 启用 Q.2
   │   └─ Q.2 仍 < 95% → ❌ 跳过 Q.Φ（开销太大）
   └─ 端侧延迟 vs P.1 < 90% → ❌ Φ 严重拖累，保留 P.1
```

##### 4.6.9.8 周期与资源

- **周期**：1 周（仅触发时启动）
- **硬件**：单卡 RTX 3090 24GB（与 §3.5 / §4.6 共享）
- **数据集**：entity-disjoint 多跳 + WikiText-103 + copy/reverse
- **依赖**：必须等 §3.5 Φ 验证通过 + §4.6.8 P.1 启用

##### 4.6.9.9 决策传递：64M → 300M

| Q.Φ 评估结果 | 300M 处理 |
|--------------|----------|
| Q.Φ 启用（延迟 ≥ 95% P.1 + OOD > 0.85）| 300M 默认开启 L-PLT + Φ |
| Q.Φ 边界（延迟 90-95% P.1 + OOD 0.75-0.85）| 300M 进一步评估 K=4 + L-PLT + Φ vs K=2 HLT-PLT |
| Q.Φ 失败（延迟 < 90% P.1 或 OOD ≤ 0.75）| 300M 仅启用 P.1（无 Φ）|

##### 4.6.9.10 关键设计原则

> **Q.Φ 是 §3.5（Φ 模块）+ §4.6（L-PLT 架构）的联合验证**，不是独立新机制。
>
> **避免叠加陷阱**：Q.Φ 失败时**保留 P.1**（L-PLT 无 Φ），**不要**继续尝试 Q.2 / Q.3 变体——边际收益递减。
>
> **共享 Φ 模块**：与 §3.5 共享同一个 Φ 通道，**不**为 Q.Φ 独立设计。
...
| **Q.1 L-PLT + Φ 输入对齐（条件启动）** | **延迟 vs P.1 ≥ 95%** | **≥ 98%（Φ 开销 ≤ 2%）** |
| Q.1 OOD 多跳准确率 vs §3.5 baseline | ≥ 0.85 | ≥ 0.9（与 §3.5 持平）|
| Q.1 桥接实体 cosine | ID ≥ 0.5 | ID ≥ 0.6（与 §3.5 持平）|
| **核心结论** | **B.2 + B.3 + B.4 三件套** + **条件启用 P.1 L-PLT** + **条件启用 Q.1 L-PLT+Φ** | **覆盖所有场景** |
